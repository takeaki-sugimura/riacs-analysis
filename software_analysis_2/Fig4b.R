...
exp_info <- read_exp_info("experiment_info_3T3.csv")
result <- build_IANode_data(exp_info,F)

area_range <- c(500,2000)
x_range <- c(-1,200)
y_range <- c(-1,200)
tdat <- removenoise(result,area_range=area_range,x_range=x_range,y_range=y_range)
tdat2 <- tdat
...
x3 <- td3$s_result1  # mean Component 1
y3 <- td3$s_result7  # std component 1
...
polygon(c(gx0,gx1,gx1,gx0),c(gy0,gy0,gy1,gy1),col=hcl(30,30,100),border=0)
lines(c(gx0,gx1),c(gy0,gy0),col=2)
